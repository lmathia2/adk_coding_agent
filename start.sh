...
usage() {
  cat <<'EOF'
Start Skein with shared local state (Codex subscription provider).

Usage:
  skein-start server [--workspace DIR] [--state-root DIR] [--model ID] [--notebook-ptc] [--trust-project] [-- SERVER_ARGS...]
  skein-start tui [--state-root DIR] [--server URL] [-- TUI_ARGS...]
  skein-start run [--workspace DIR] [--state-root DIR] [--model ID] [--trust-project] [-- TUI_ARGS...]
...
SKEIN_STATE_ROOT defaults to ~/.local/state/skein.
SKEIN_SERVER_URL configures only the TUI; default ws://127.0.0.1:8765/v1/agent.
For a custom YAML/Gemini provider use skein serve --config FILE directly.
EOF
}
die() { printf 'error: %s\n' "$1" >&2; exit "${2:-1}"; }
...
workspace=$(pwd -P)
state_root=${SKEIN_STATE_ROOT:-${HOME}/.local/state/skein}
server_url=${SKEIN_SERVER_URL:-ws://127.0.0.1:8765/v1/agent}
model=
trust_project=0
notebook_ptc=0
while [ "$#" -gt 0 ]; do
...
if [ -n "${SKEIN_TUI_COMMAND:-}" ]; then
  tui_command=$SKEIN_TUI_COMMAND
elif [ -x "$project_root/clients/terminal/skein-tui" ]; then
  tui_command=$project_root/clients/terminal/skein-tui
elif command -v skein-tui >/dev/null 2>&1; then
  tui_command=skein-tui
else
...
printf '%s\n' \
  '  Environment set for TUI: SKEIN_TOKEN (read from the auth token file)' \
  '  Environment set for TUI: SKEIN_STATE_ROOT (resolved state directory)' \
  '  The token value is never printed or passed on the command line.'
SKEIN_TOKEN=$token
SKEIN_STATE_ROOT=$state_root
export SKEIN_TOKEN SKEIN_STATE_ROOT
if [ "$mode" = tui ]; then
